...
function nav() {
  ...
  while (i <= sections.length) {
    ...
    list.innerHTML = `<a class="menu__link" id="l${
      sections[i - 1].id
    }" href="#${sections[i - 1].id}"> ${name} </a>`;
...
//smooth scrolling after clicking
function smoothScroll() {
  document.querySelectorAll("a").forEach(function (link) {
    // looping over links
    link.addEventListener("click", function (element) {
      //adding click listener
      element.preventDefault(); //prevent a link from opening the URL
      const scroll = document.getElementById(
        this.getAttribute("href").substring(1)
      );
      scroll.scrollIntoView({
        behavior: "smooth", //scrolling behavior
      });
    });
  });
}
